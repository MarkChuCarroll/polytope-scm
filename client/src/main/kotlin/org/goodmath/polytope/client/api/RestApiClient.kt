...
class RestApiClient(
    ...
    companion object {
        private val authClient = HttpClient(CIO) { install(ContentNegotiation) { json() } }

        /**
         * A helper function for the auth functionality - this uses the login request method of the
         * server to get a JWT auth token that can be used as a bearer authentication.
         */
        suspend fun getAuthToken(serverUrl: Url, userId: String, password: String): BearerTokens {
            val response =
                    authClient.post(serverUrl) {
                        url { appendPathSegments("login") }
    ...
    private val client =
            HttpClient(CIO) {
                ...
                install(Auth) {
                    bearer {
                        loadTokens { getAuthToken(serverUrl, userId, password) }
                        refreshTokens { getAuthToken(serverUrl, userId, password) }
    ...
    /**
     * A convenience wrapper, which runs an HTTP request, and unpacks the response, turning 2xxs
     * into return values, and throwing an appropriate exception for anything else.
     */
    private suspend inline fun <reified T> runWithErrorHandling(requester: () -> HttpResponse): T {
    ...
    suspend fun userList(): UserListResponse {
        return runWithErrorHandling {
            client.get(serverUrl) { url { appendPathSegments("users") } }
        }
    }
    ...
    suspend fun userGrant(userId: String, permitted: List<Action>): User {
        return runWithErrorHandling {
            client.put(serverUrl) {
                ...
                url { appendPathSegments("users", userId) }
                setBody(UserUpdateRequest(kind = UserUpdateRequest.Kind.Grant, actions = permitted))
            }
        }
    }
    ...
    suspend fun userRevoke(userId: String, revoked: List<Action>): User {
        return runWithErrorHandling {
            client.put(serverUrl) {
                ...
                url { appendPathSegments("users", userId) }
                setBody(UserUpdateRequest(kind = UserUpdateRequest.Kind.Revoke, actions = revoked))
            }
        }
    }
    suspend fun userDeactivate(userId: String) {
        return runWithErrorHandling {
            client.put(serverUrl) {
                ...
                url { appendPathSegments("users", userId) }
                setBody(UserUpdateRequest(kind = UserUpdateRequest.Kind.Deactivate))
            }
        }
    }
    suspend fun userReactivate(userId: String) {
        return runWithErrorHandling {
            client.put(serverUrl) {
                ...
                url { appendPathSegments("users", userId) }
                setBody(UserUpdateRequest(kind = UserUpdateRequest.Kind.Reactivate))
            }
        }
    }
    ...
    suspend fun userCreate(
            ...
            userEmail: String,
            userPassword: String,
            permitted: List<Action>
    ): User {
        return runWithErrorHandling {
            client.post(serverUrl) {
                headers { contentType(ContentType.Application.Json) }
                url { appendPathSegments("users") }
                setBody(UserCreateRequest(userId, userFullName, userEmail, userPassword, permitted))
            }
        }
    }
    ...
    suspend fun projectList(): ProjectListResponse {
        return runWithErrorHandling {
            client.get(serverUrl) {
                headers { contentType(ContentType.Application.Json) }
                url { appendPathSegments("projects") }
            }
        }
    }
    ...
    suspend fun projectGet(name: String): Project {
        return runWithErrorHandling {
            client.get(serverUrl) {
                headers { contentType(ContentType.Application.Json) }
                url { appendPathSegments("projects", name) }
            }
        }
    }
    ...
    suspend fun historyList(project: String): HistoryListResponse {
        return runWithErrorHandling {
            client.get(serverUrl) {
                headers { contentType(ContentType.Application.Json) }
                url { appendPathSegments("projects", project, "histories") }
            }
        }
    }
    ...
    suspend fun historyCreate(
            project: String,
            historyName: String,
            description: String,
            parentHistory: String,
            parentHistoryIndex: Int? = null
    ): History {
        return runWithErrorHandling {
            client.post(serverUrl) {
                headers { contentType(ContentType.Application.Json) }
    ...
    suspend fun historyGet(project: String, history: String): History {
        return runWithErrorHandling {
            client.get(serverUrl) {
                headers { contentType(ContentType.Application.Json) }
                url { appendPathSegments("projects", project, "histories", history) }
            }
        }
    }
    ...
    suspend fun stepsList(project: String, history: String): HistoryStepsResponse {
        return runWithErrorHandling {
            client.get(serverUrl) {
                headers { contentType(ContentType.Application.Json) }
                url { appendPathSegments("projects", project, "histories", history, "steps") }
            }
        }
    }

    suspend fun abortChange(
            project: String,
            history: String,
            change: String,
            reason: String
    ...
    suspend fun workspacesList(project: String): WorkspaceListResponse {
        return runWithErrorHandling {
            client.get(serverUrl) {
                headers { contentType(ContentType.Application.Json) }
                url { appendPathSegments("projects", project, "workspaces") }
            }
        }
    }
    ...
    suspend fun workspaceGet(project: String, wsName: String): Workspace {
        return runWithErrorHandling {
            client.get(serverUrl) {
                headers { contentType(ContentType.Application.Json) }
                url { appendPathSegments("projects", project, "workspaces", wsName) }
            }
        }
    }
    ...
    suspend fun workspaceUtd(project: String, wsName: String): Boolean {
        return runWithErrorHandling {
            client.get(serverUrl) {
                headers { contentType(ContentType.Application.Json) }
                url { appendPathSegments("projects", project, "workspaces", wsName, "utd") }
            }
        }
    }

    suspend fun workspaceCreateChange(
            project: String,
            wsName: String,
            historyName: String,
            changeName: String,
    ...
    suspend fun workspaceOpenChange(
            project: String,
            wsName: String,
            historyName: String,
            changeName: String
    ...
    suspend fun workspaceListPaths(project: String, wsName: String): PathListResponse {
        return runWithErrorHandling {
            client.get(serverUrl) {
                headers { contentType(ContentType.Application.Json) }
                url { appendPathSegments("projects", project, "workspaces", wsName, "paths") }
            }
        }
    }

    suspend fun workspaceAddFile(
            project: String,
            wsName: String,
            path: String,
    ...
    suspend fun workspaceDeleteFile(project: String, wsName: String, path: String): List<String> {
        return runWithErrorHandling {
            client.delete(serverUrl) {
                headers { contentType(ContentType.Application.Json) }
                url {
    ...
    suspend fun workspaceModifyFile(
            project: String,
            wsName: String,
            path: String,
            contents: String
    ...
    suspend fun workspaceGetFiles(
            project: String,
            wsName: String,
            paths: List<String>
    ): List<WorkspaceFileContents> {
        return runWithErrorHandling {
            client.post(serverUrl) {
                headers { contentType(ContentType.Application.Json) }
                url { appendPathSegments("projects", project, "workspaces", wsName, "multi") }
                setBody(WorkspaceGetMultiRequest(paths))
            }
        }
    }
    suspend fun workspaceSave(
            project: String,
            wsName: String,
            description: String,
    ...
    suspend fun workspaceDeliver(project: String, wsName: String, description: String): Workspace {
        return runWithErrorHandling {
            client.post(serverUrl) {
                headers { contentType(ContentType.Application.Json) }
                url {
    ...
    suspend fun workspaceIntegrateDiff(
            project: String,
            wsName: String,
    ...
    suspend fun workspaceIntegrateChange(
            project: String,
            wsName: String,
            history: String,
            change: String
