...
class ArtifactStash(
    ...
    private fun fetchParents(
        ...
        project: String,
        artId: Id<Artifact>,
        verId: Id<ArtifactVersion>): List<Id<ArtifactVersion>> {
    ...
    fun versionIsAncestor(auth: AuthenticatedUser, project: String,
                          ...
                          maybeDescendant: Id<ArtifactVersion>): Boolean {
        val queue = ArrayDeque<Id<ArtifactVersion>>()
    ...
    /**
     * Get a nearest common ancestor of two versions.
     *
     ...
     *
     * If there's more than one thing in the intersections, then all of the elements of
     * the intersection are equally distance from the two versions, so we can arbitrarily
     * chose any of them as a NCA.
     *
     * @param auth
     * @param project
     * @param artifactId
