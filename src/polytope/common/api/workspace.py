...
from polytope.common.stashable.workspace import WorkspaceDescriptor, Workspace
...
class WsResetReq(NamedTuple):
    reason: str
    step_index: int | None

    @classmethod
    def from_dict(cls, d: JDict) -> "WsResetReq":
        return cls(
            reason=d["reason"],
            step_index=d["step_index"] if "step_index" in d else None,
        )
