...
from polytope.common.api.history import (
    ChangeListResponse,
    HistoryCreateRequest,
    HistoryListResponse,
    SavesListResponse,
)
from polytope.common.api.projects import ProjectCreateRequest, ProjectListResponse
...
class ProjectResource(PtResource):
    ...
    @with_auth
    def on_get(
        self, auth: AuthenticatedUser, req: Request, resp: Response, project_name: str
    ) -> None:
...
class ProjectHistoriesResource(PtResource):
    ...
    @with_auth
    def on_get(
        self, auth: AuthenticatedUser, req: Request, resp: Response, project: str
    ) -> None:
        histories = self.depot.history_stash.list_histories(auth, project)
        resp.status = falcon.HTTP_200
        resp.content_type = "application/json"
        resp.media = HistoryListResponse(project=project, histories=histories).to_dict()
...
class ProjectHistoryResource(PtResource):
    ...
    def on_get(
        ...
        history_obj = self.depot.history_stash.retrieve_history(auth, project, history)
        resp.status = falcon.HTTP_200
        resp.content_type = "application/json"
...
class ProjectHistoryStepsResource(PtResource):
    ...
    def on_get(
        ...
        steps = self.depot.history_stash.list_history_steps(auth, project, history)
        resp.status = falcon.HTTP_200
        resp.content_type = "application/json"
        resp.media = {
...
class ProjectHistoryChangesResource(PtResource):
    ...
    def on_get(
        ...
        changes = self.depot.change_stash.list_changes(
            auth, project, history, ChangeStatus.Open
        )
...
class ProjectHistoryChangeResource(PtResource):
    ...
    def on_get(
        ...
        ch = self.depot.change_stash.retrieve_change_by_name(
            auth, project, history, change_name
        )
